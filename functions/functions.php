...
function create_user($first_name, $last_name, $username, $email, $password) {
    ...
    $sql .= "VALUES('$first_name','$last_name','$username','uploads/deafault.jpg','$email','$password')";
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if(empty($errors)) {
            if(user_login($email, $password)) {
                ...
                $errors[] = "Your email or password is incorect. Please try again";
            }
        }
...
function user_profile_image_upload() {
    if($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if($uploadOK == 0) {
            set_message('Error upoading file: ' . $error);
        } else {
...
function fetch_all_posts() {
    ...
    if($result->num_rows > 0){
        while($row = $result->fetch_assoc()) {
            ...
            echo "<div class='post'>
                        <p><img src='" . $user['profile_image'] . "' alt='Profile-picture'><i><b>" . $user['first_name'] . " " . $user['last_name'] . "</b></i><p>
                        <p>" . $row['content'] . "</p>
